...
class WashingMachineDevice extends BaseDevice {
     ...
     * Kandidaten:
     *  - Start = jede Stunden-Grenze >= notBefore
     *  - Start = (jede Stunden-Grenze) - minutes   (Ende an Grenze)
     *  - Start = notBefore selbst (falls passend)
     *
     * Rückgabe: { startTime: Date, endTime: Date, avgPrice: number } oder null
     */
    _findCheapestWindowByMinutes(prices, minutes, notBefore = new Date()) {
        ...
        // notBefore selbst testen (falls es reinpasst)
    ...
    async planWashingProgram({ program, duration, dryerNeeded, dryerDuration }) {
        ...
        if (dryerNeeded) {
            ...
            const combined = this._findCheapestWindowByMinutes(prices, washMinutes + dryMinutes, now);

            // Split: zuerst Waschen, danach Trockner ab Wasch-Ende
            const washWin = this._findCheapestWindowByMinutes(prices, washMinutes, now);
            let split = null;
            if (washWin) {
                const dryerWin = this._findCheapestWindowByMinutes(prices, dryMinutes, washWin.endTime);
                if (dryerWin && dryerWin.startTime >= washWin.endTime) {
                    ...
                    split = {
                        variant: "split",
                        withDryer: true,
                        startTime: washWin.startTime,
                        endTime:   dryerWin.endTime,
                        ...
                        avgPriceWash: washWin.avgPrice,
                        avgPriceDryer: dryerWin.avgPrice,
                        _combinedAvg: splitAvg
                    };
                }
            }
            ...
            const combinedResult = combined ? {
                ...
                avgPriceWash: combined.avgPrice,
                avgPriceDryer: combined.avgPrice,
                _combinedAvg: combined.avgPrice
            } : null;

            // Entscheidung: kleinster gewichteter Durchschnitt
            if (combinedResult && split) {
                result = (combinedResult._combinedAvg <= split._combinedAvg) ? combinedResult : split;
            } else {
                result = combinedResult || split || null;
            }

        } else {
            // Nur Waschen
            const win = this._findCheapestWindowByMinutes(prices, washMinutes, now);
            if (!win) {
                this.adapter.log.warn(`${this.name}: No suitable window for washing found`);
...
/*        await this.adapter.setStateAsync(`devices.${this.id}.planned.start`, result.startTime.toISOString(), true);
        ...
        // 4) Benachrichtigung (klar & ehrlich)
