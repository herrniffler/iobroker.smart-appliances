...
class BaseDevice {
    ...
    // Helpers to build object paths under devices.<types>.<id>
    channelId() {
        // e.g. devices.washings? better pluralize by adding 's'
        return `devices.${this.type}s.${this.id}`;
    }
    ...
    async setStateAsync(name, value, ack = false) {
        return this.adapter.setStateAsync(this.stateId(name), value, ack);
    }

    async getStateAsync(name) {
        return this.adapter.getStateAsync(this.stateId(name));
    }

    async setObjectNotExistsAsync(id, obj) {
        return this.adapter.setObjectNotExistsAsync(id, obj);
    }
    ...
    async subscribe() {
        ...
        await this.adapter.subscribeStatesAsync(`${this.channelId()}.*`);
    }

    async initializeStates() {
        // Initialize basic states
        await this.setStateAsync("running", false, true);
        await this.setStateAsync("scheduled", false, true);
        await this.setStateAsync("startTime", "", true);
    ...
    async restoreScheduledOperations() {
        ...
        try {
            const scheduledState = await this.getStateAsync(`scheduled`);
            const startTimeState = await this.getStateAsync(`startTime`);
    ...
    handlesState(stateId) {
        return this.subscriptions.includes(stateId) ||
               stateId.startsWith(`${this.adapter.namespace}.${this.channelId()}.`);
    }

    async onStateChange(id, state) {
        // Base generic scheduling reactions (only if enabled)
        if (this.genericScheduling && state) {
            const isStartTime = id.endsWith(`${this.channelId()}.startTime`);
            const isScheduled = id.endsWith(`${this.channelId()}.scheduled`);
            ...
            if (!state.ack) {
                ...
                if (isScheduled && state.val === false) {
                    ...
                    const scheduled = await this.getStateAsync(`scheduled`);
    ...
    async _handleManualStartTimeInput(value) {
        ...
        await this.setStateAsync(`startTime`, date.toISOString(), true);
        const scheduledState = await this.getStateAsync(`scheduled`);
    ...
    async _handleManualScheduledInput(val) {
        if (val === true) {
            ...
            await this.setStateAsync(`scheduled`, true, true); // ack
            const delay = date.getTime() - Date.now();
            if (delay <= 0) {
                await this._executeScheduledStart();
            } else {
    ...
    async scheduleStartAt(startTime) {
        ...
        await this.setStateAsync(`startTime`, start.toISOString(), true);
        await this.setStateAsync(`scheduled`, true, true);
    ...
    async cancelScheduledStart(sendNotification = true) {
        ...
        const sched = await this.getStateAsync(`scheduled`);
