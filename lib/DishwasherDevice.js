...
class DishwasherDevice extends BaseDevice {
    ...
    async finishDevice() {
        ...
        // Clear all timers to prevent duplicate triggers
        this.clearEndTimers();
        this.clearDryReminder();
        ...
        try {
            this.adapter.log.info(`${this.name}: Creating ToDoist task (unload dishwasher)`);
            await this.adapter.createTodoistTask({
                content: "Spülmaschine ausräumen"
                // projectId, dueString, priority werden aus der Adapter-Konfiguration übernommen
            });
        } catch (e) {
    ...
    async executeScheduledStart() {
        ...
        // Clear flag after a reasonable time (300 seconds) to allow normal detection again
        setTimeout(() => {
            this.automaticStartInProgress = false;
